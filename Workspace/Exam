...
- E. (P ∨ Q) → (P ∧ Q) NOT VALID
⟦P⟧ = false, [Q] = true, P ∨ Q is true, but P ∧ Q is false
...
(
  ...
  intro hporq hnp2nq
  /-
  You can't construct a proof of false in lean

  This proposition is not provable because
  the two assumptions do not always contradict each other.
  example:
    Let P = False, Q True
    Then:
      P ∨ Q     is True
  ...
  For Example:
    ...
    Then:
      P ∨ Q     is True
     ¬P → ¬Q    is True  (false implies anything)
  Both premises hold but it doesn't show false
  This statement cannot be proven constructively or classically
  -/
  sorry
  )
)
...
(
  ...
  Reason:
    To prove (P ∨ Q) ∧ (¬P → Q), we must show both parts:
    1. P ∨ Q    P or Q is true
    2. ¬P → Q   If P is false, Q is true
    with no assumptions about P or Q, we cannot guarantee either.
    example:
      Let P = False and Q = False.
      Then:
        P ∨ Q --- False
        ¬P → Q --- True → False
  The first part is false, so the conjunction fails.
  the statement is not provable constructively or classically
  -/
  sorry
)
...
Within constructive logic, propositions can only be proved using
  direct construction, not by contradiction.
While in classical reasoning one can assume ¬¬P → P,
if you use constructive logic you cannot. In this case we can
directly derive a contradiction from P, so it works,
but in gneral formulas that rely on proof by
contradiction are valid classically but not
constructively.
